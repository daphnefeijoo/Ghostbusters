...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for elim_var in eliminationOrder:
            eliminated = joinFactorsByVariable(allCPTS, elim_var)
            allCPTS = eliminated[0]
            if len(eliminated[1].unconditionedVariables()) > 1:
                allCPTS += [eliminate(eliminated[1], elim_var)]
        returner = normalize(joinFactors(allCPTS))
        return returner
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in allpos:
            posprob = self.getObservationProb(observation, pacpos, pos, jailpos)
            self.beliefs[pos] = self.beliefs[pos]*posprob
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        # Update the beliefs with the new beliefs
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        fillpositions = DiscreteDistribution()
        for pos in positions:
            fillpositions[pos] = 1
        self.particles = [fillpositions.sample() for _ in range(particlenum)]
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for potentialghostpos in self.particles:
            prob = self.getObservationProb(observation, pacpos, potentialghostpos, jailpos)
            weights[potentialghostpos] += prob
        if weights.total() == 0:
            self.initializeUniformly(gameState)
        else:
            weights.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        newparticles = []
        for oldghostpos in self.particles:
