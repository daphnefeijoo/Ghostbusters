...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for ghost in livingGhostPositionDistributions:
            maxprob = max(ghost.items(), key = lambda x: x[1])
            ghostpositions.append(maxprob[0])
        closest_ghost = [ghostpositions[0], self.distancer.getDistance(pacmanPosition, ghostpositions[0])]
        for ghost in ghostpositions:
            dist = self.distancer.getDistance(pacmanPosition, ghost)
            if dist < closest_ghost[1]:
                closest_ghost = [ghost, dist]
        bestaction = [0, float('inf')]
        for action in legal:
            newpacpos = Actions.getSuccessor(pacmanPosition, action)
            newpacdist = self.distancer.getDistance(newpacpos, closest_ghost[0])
            if newpacdist < bestaction[1]:
                bestaction = [action, newpacdist]
        return bestaction[0]
        "*** END YOUR CODE HERE ***"
